    ...
    if MAX_BATCH_SIZE > 0:
        ...
        class GLiNERGuardGrpcBatched:
            ...
            async def _handle_batch(
                self, requests: list[gliner_guard_pb2.PredictRequest],
            ) -> list[gliner_guard_pb2.PredictResponse]:
                ...
                for result in results:
                    ...
                    for label, values in result.get("entities", {}).items():
                        entities_map[label] = gliner_guard_pb2.EntityList(
                            values=values
                        )
                    responses.append(gliner_guard_pb2.PredictResponse(
            ...
            async def Predict(
                self, request: gliner_guard_pb2.PredictRequest,
            ) -> gliner_guard_pb2.PredictResponse:
